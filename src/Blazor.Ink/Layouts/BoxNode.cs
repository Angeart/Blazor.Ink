...
public class BoxNode : NodeBase<Box>
{
    ...
    protected override void ApplyLayoutImpl()
    {
        unsafe
        {
            ...
            if (Component.MinWidth is not null) Node->SetMinWidth(Component.MinWidth.Value);

            if (Component.MinHeight is not null) Node->SetMinHeight(Component.MinHeight.Value);
            // Flex
